...
def main(args):
    ...
    load_args = [(scene_id, data_root, cfar_configs_name, args) for scene_id in scene_ids]
    with mp.Pool(50) as pool:
        loaded = pool.map(get_data, load_args)
    X_tuple, centroids_tuple, length_tuple, scene_ids_tuple = zip(*loaded)
    data = []
    for i, scene_id in enumerate(scene_ids_tuple):
        X, centroids, length = X_tuple[i], centroids_tuple[i], length_tuple[i]
...
def get_data(loaded_args):
    scene_id, data_root, cfar_configs_name, args = loaded_args
    logger.info(f'Starting on {scene_id}')
    ...
    if not (os.path.exists(centroids_filepath) and os.path.exists(X_filepath)) or args.overwrite:
        logger.info(f'Running CFAR.')
        try:
            X, centroids, length = cfar_inference(data_folder=os.path.join(data_root, scene_id),
                                                  configs_name=cfar_configs_name)
    ...
    return X, centroids, length, scene_id
